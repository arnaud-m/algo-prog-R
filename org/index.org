...
**** Calcul de la note de contrôle continu
  ...
  #+BEGIN_SRC R :var exercices=exercices :var projets=projets :session grade
    ...
    file <- '~/downloads/SPUA31 Notes-20201207_1603-comma_separated.csv'
    df <- read.csv(file, stringsAsFactors = FALSE, na.strings = c("-", "NA"), header = TRUE)

    FindColumns <-function(df, patterns) sapply(patterns, function(x) grep(x, colnames(df)))
    }
  #+END_SRC
**** Analyse des notes
...
**** 2019-2020
  #+BEGIN_SRC R :var exercices=exercices :var projets=projets :session grade
    ...
    activities <- data.frame(
      name = make.names(c(UnLink(c(exercices$Problème, projets$Activité)))),
      grade = c(exercices$Barème, projets$Barème),
    )

    ## Load grades from moodle
    file <- '~/downloads/SPUA31 Notes-20201207_1603-comma_separated.csv.csv'
    df <- read.csv(file, stringsAsFactors = FALSE, na.strings = c("-", "NA"), header = TRUE)

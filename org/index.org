...
**** Calcul des notes
  ...
  #+BEGIN_SRC R :var exercices=exercices :var projets=projets :session grade
    ...
    file <- '~/downloads/SPUA31 Notes-20201221_1342-comma_separated.csv'
    ...
    ## Grade NOTE
    ComputeFinalGrade <- function(ct, cc, bo) pmin(pmax(ct, 0.6 * ct + 0.4 * cc ) + bo, 20)
...
**** Analyse des notes
  ...
  #+BEGIN_SRC R :session grade
    ...
    file <- '~/downloads/SPUA31 Notes-20201221_1402-comma_separated.csv'
    df <- read.csv(file, stringsAsFactors = FALSE, na.strings = c("-", "NA"), header = TRUE)

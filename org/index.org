...
***** Session 1
  #+BEGIN_SRC R :var exercices=exercices :var projets=projets :session grade
      ...
      file <- "~/download/SPUA31 Notes-20240123_1220-comma_separated.csv"
      ...
      ## Grade CC
      indA <- (grepl("^Devoir\\.", colnames(df)) | grepl("(RWC2023|LUHN10|LUHNN)", colnames(df))) & ! grepl("Bug.Bounty", colnames(df))

      SumTwoBestGrades <- function(x) sum(tail(sort(x), 2), na.rm = TRUE)
      gradeA <- apply(df[, indA], 1, SumTwoBestGrades)
      partiel <- "Partiel.du.12.10..Brut."
      examen <-  "CT..Brut."
...
**** Analyse des notes
  ...
  #+BEGIN_SRC R :session grade :results none
    file <- '~/download/SPUA31 Notes-20240123_1230-comma_separated.csv'
    df <- read.csv(file, stringsAsFactors = FALSE, na.strings = c("-", "NA"), header = TRUE)
    ...
    toptitle <- textGrob(sprintf("SPUA31 - Algo & Prog avec R - 2023/2024 - %d étudiants - %d absents - %d acquis", nrow(grades), sum(is.na(grades$NOTE)), sum(grades$NOTE >= 10, na.rm=T)), gp=gpar(fontsize=50,font=2))

    p <- grid.arrange(p2, p1, p3, p4, nrow = 2, top = toptitle)
    ggsave('SPUA31-statistics-2023-2024.pdf', plot = p, device = "pdf", width = 40, height = 15)
...
*** Ressources BU
   - [[https://catalogue.bu.univ-cotedazur.fr/permalink/f/11hf8mm/33UCA_KOHA75121

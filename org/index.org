...
* Intervenants
 - Arnaud Malapert (responsable), Riccardo Di Dio, Oussama Sabri.
...
** Salles
   ...
   Les initiales de l'intervenant sont indiqués entre parenthèses.

  | 09h15-11h45 | Salle     | 13h15-15h45 | Salle    |
  |-------------+-----------+-------------+----------|
  | *Groupe 1*  | 315 (RDD) | *Groupe 4*  | 201 (AM) |
  | *Groupe 2*  | 316 (AM)  |             |          |
  | *Groupe 3*  | 317 (OS)  |             |          |
...
** Activités
   ...
   Choisissez votre binôme sur [[https://lms.univ-cotedazur.fr/course/view.php?id=10988][moodle]] pour toutes vos activités !
...
*** Exercices UCAnCODE
 ...
 | [[https://51364960.widgets.sphere-engine.com/lp?hash=DzcCU8OIxy][HASH1]]             |     12 | 28/10 - 29/10 |
 | [[https://51364960.widgets.sphere-engine.com/lp?hash=LlOvxlZVwc][HASH2]]             |     12 | 11/11 - 12/11 |
 | HASH 1 ET 2       |     14 |               |
 | MULT3             |  Bonus | 12-11 - 19/11 |
 | [[https://51364960.widgets.sphere-engine.com/lp?hash=1t9d56vSV2][GEOHASH]]           |    17+ | 18/11 - 26/11 |
 | [[https://51364960.widgets.sphere-engine.com/lp?hash=IG2VzE9Kzo][PAIRS]]             |     14 | 03/12         |
...
**** Calcul des notes
  ...
  #+BEGIN_SRC R :var exercices=exercices :var projets=projets :session grade
      ...
      file <- "~/download/SPUA31 Notes-20220121_1249-comma_separated.csv"
      ...
      ## Grade CC
      indA <- (grepl("^Devoir\\.", colnames(df)) | grepl("(HASH|PAIRS)", colnames(df))) & ! grepl("Bug.Bounty", colnames(df))
...
**** Analyse des notes
  ...
  #+BEGIN_SRC R :session grade :results none
    ...
    file <- '~/download/SPUA31 Notes-20220121_1331-comma_separated.csv'
    df <- read.csv(file, stringsAsFactors = FALSE, na.strings = c("-", "NA"), header = TRUE)
    ...
    toptitle <- textGrob(sprintf("SPUA31 - Algo & Prog avec R - 2021/2022 - %d étudiants - %d absents - %d acquis", nrow(grades), sum(is.na(grades$CC)), sum(grades$NOTE >= 10, na.rm=T)), gp=gpar(fontsize=50,font=2))

    p <- grid.arrange(p2, p1, p3, p4, nrow = 2, top = toptitle)
    ggsave('SPUA31-grades-2021-2022.pdf', plot = p, device = "pdf", width = 40, height = 15)

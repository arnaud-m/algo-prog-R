...
** Calcul du résultat par une fonction
   Définissez la fonction ~Somme2Max(x,y,z)~ prenant trois entiers x, y, z et retournant la somme des deux plus grands.
     ...
     2. Écrivez une seconde version utilisant la fonction prédéfinie ~max~.
     3. Écrivez une troisième version utilisant la fonction prédéfinie ~min~.
...
* Calcul de la note finale                                     :UCANCODE:KEY:

Chaque année, votre professeur est assailli de questions existentielles sur le calcul de la note finale.
Plutôt que de sombrer dans la répétition éternelle, il a flairé une excellente occasion pédagogique !

Votre mission est d'écrire un programme qui calcule la note finale.
...
- ~ca2~ : l'autre des deux meilleures notes d'activité.

Remarquez que les arguments peuvent avoir la valeur ~NA~ si la note associée est manquante.
Si la note finale ne peut pas être calculée, car la note d'examen terminal et de seconde chance sont manquantes, alors la fonction renvoie ~NA~.

 Testez plus finement votre fonction grâce à l'[[https://51364960.widgets.sphere-engine.com/lp?hash=5Mlcr3EdrZ][exercice UCAnCODE]].
...
#+BEGIN_SRC R :exports both :session note
...
/Conseils/ :
- Définir une fonction auxiliaire ~ArrondirNote(x)~, car il faut réaliser plusieurs arrondis.
- Définir une fonction auxiliaire ~CalculerControleContinu(ct1, cp, ca1, ca2)~ pour remplacer des calculs complexes par un appel de fonction.
- Utiliser les fonctions ~sum~ et ~max~ avec l'argument ~na.rm~

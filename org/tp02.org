...
* Génération aléatoire d'un nombre                                      :KEY:
  ...
  Nous allons utiliser une fonction ~sample~ tirant au hasard des éléments dans une collection, on dit aussi aléatoires. Pour afficher un entier aléatoire entre [1,100], tapez l'instruction ci-dessous.
...
** Dans un ensemble
   ...
   3. Testez la commande ~table(replicate(10, LasVegas()))~ ? À quoi sert-elle ?

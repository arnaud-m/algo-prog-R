...
*** Virginie lance trois dés numérotés de 1 à 6 :
    1. Si elle obtient une somme de 18, elle gagne 50 euros,
    2. entre 10 et 17, elle gagne 5 euros,
    3. sinon elle ne gagne rien.


*** Écrivez une fonction ~JeuHasard~ utilisant la fonction ~sample~ pour simuler un lancer de dés, puis renvoyant le gain. Pour faire la somme des valeurs renvoyées par ~sample~, utilisez la fonction ~sum~ ainsi : ~sum(sample(...))~.
 #+BEGIN_SRC R :results output silent :session jeuHasard
   JeuHasard <- function() {
...
*** Quelle est la probabilité de gagner 50 euros ? Utiliser l'appel ~table(replicate(1000, JeuHasard()))~ pour vérifier le hasard engendré par votre fonction.

*** Écrire une fonction ~SimulationJeuHasard(n)~ qui simule une stratégie où Virginie joue jusqu'à ce que son gain total dépasse le montant ~n~ en affichant un message après chaque partie.

   #+BEGIN_SRC R :results output silent :session jeuHasard
     SimulationJeuHasard <- function(n) {
       gain <- 0
       partie <- 0
       cat(sprintf("|%6s|%5s|%5s|\n", "Partie", "Gain", "Total"))
       while(gain < n) {
         g <- JeuHasard()
         gain <- gain + g
         partie <- partie + 1
         cat(sprintf("|%6d|%5d|%5d|\n", partie, g, gain))
       }
     }
    #+END_SRC

   #+BEGIN_SRC R :exports both :session jeuHasard
     SimulationJeuHasard(25)
    #+END_SRC
...
*** Quelle est l'espérance de gain ? Écrire une fonction ~EstimationEsperance(n)~ qui calcule l'espérance de gain obtenue expérimentalement après ~n~ parties. Proposer un tarif pour jouer à ce jeu. Justifier.

     #+BEGIN_SRC R :session jeuHasard
       ## Estimation de l'espérance par simulation
       EstimationEsperance <- function(n) {
         return(mean(replicate(n, JeuHasard())))
       }
       cat("Esperance simulée :", EstimationEsperance(1000), "\n")

       ## Calcul théorique de l'espérance
       CalculEsperance <- function() {
         tirages <- expand.grid(1:6, 1:6, 1:6)
         sommes <- rowSums(tirages)
         ##  print(cbind(tirages, sommes))
         prob50 <-  mean(sommes == 18)
         prob5 <- mean(sommes >= 10 & sommes <= 17)
         return( 50*prob50 + 5*prob5)
       }
       cat("Esperance théorique", CalculEsperance(), '\n')
     #+END_SRC

     #+RESULTS:
     : Esperance simulée : 3.5

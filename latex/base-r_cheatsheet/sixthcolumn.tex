...
\begin{seeblock}{Strings}{stringr}
  ...
  \begin{tabular}{c >{\footnotesize} >{\hspace{-0.05\linewidth}}p{0.5\linewidth}}
    \inline{cat(x, y, sep = ' ')} & Join and print multiple vectors together.\\
    \inline{cat(x, collapse = ' ')} & Join and print elements of a vector together.\\
    ...
    % \inline{sprintf(fmt, ...)} & Return a character vector containing a formatted combination of text and variable values.\\
    \inline{utf8ToInt(x)} & ASCII code of character \inline{x}.\\
    \inline{intToUtf8(x)} & Character with ASCII code \inline{x}.
  \end{tabular}
  \vskip-1.5ex
\end{seeblock}

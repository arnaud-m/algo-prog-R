...
\begin{block}{Vectors}
  \begin{subblock}{Creating Vectors}
    ...
    \begin{tableau}{| >{\color{black}}m{2.95cm} | >{\color{white}\cellcolor{codebg}}m{1.55cm} | >{\color{black}\centering\arraybackslash}m{2.2cm} |}
      \hline
      \inl{c(2, 4, 6)} & \inl{2 4 6} & Join elements\\\hline% into a vector\\\hline
      ...
      \inl{seq_along(c('a','b'))} & \inl{1 2} & Indexes of a vector\\\hline
    \end{tableau}
    \vskip-2ex
  \end{subblock}
    \vskip-0.5ex
  ...
  \begin{subblock}{Selecting Vector Elements}
    ...
    \begin{tabular}{>{\centering}m{0.35\linewidth} >{\centering\arraybackslash}m{0.6\linewidth}}
      ...
      \inlc{head(x, 3)}      & The first three elements.           \\
      \inlc{tail(x, 3)}      & The last three elements.            \\
      \multicolumn{2}{c}{\textcolor{gray}{\textbf{By Value}}}      \\
      \inlc{x[x == 10]}      & Element which are equal to 10.      \\
      \inlc{x[which(}        & Element which are                   \\[-2mm]
      \inlc{x == 10)]}       & equal to 10.                        \\
      \inlc{x[x < 0]}        & All elements less than zero.        \\
      \inlc{x[x \%in\%}      & Elements in the set                 \\[-2mm]
      \inlc{c(1, 2, 5)]}     & \{1, 2, 5\}.                        \\
  ...
  \begin{subblock}{Testing Vector Elements}
    \renewcommand{\arraystretch}{1.411}\hspace{-17.5pt}
    \begin{tabular}{>{\centering}m{0.3\linewidth} >{\centering\arraybackslash}m{0.66\linewidth}}
      \inlc{all(x == 5)} & All elements are equal to 5? \\
      \inlc{any(x < 10)} & An element is less than 10? \\
    \end{tabular}
  \end{subblock}
\end{block}
...
\begin{frame}[fragile]
  ...
  \tikzstyle{block} = [draw, fill=gray!10, rectangle, minimum height=3em, minimum width=6em]
...
\begin{tikzpicture}[auto, node distance=2cm,>=latex']
    ...
    \node [block, align = left] (INIT) {\lstinline[basicstyle=\ttfamily]{i <- 0}\\\lstinline[basicstyle=\ttfamily]{acc <- 0}};
    \node [sum, right of=INIT, node distance=2cm] (INV) {INV};
    \node [block, diamond, right of=INV, , node distance=2cm] (TERM) {\lstinline[basicstyle=\ttfamily]{i < n ?}};
    ...
    \node [block, align = left, below of=TERM, node distance=2cm] (ITER) {\lstinline[basicstyle=\ttfamily]{i <- i + 1}\\\lstinline[basicstyle=\ttfamily]{acc <- acc + i}};% {$\mathtt{i} = \mathtt{i} + 1$\\$\mathtt{acc}=\mathtt{acc}+\mathtt{i}$};

    % Once the nodes are placed, connecting them is easy.
    \draw [draw,->] (INIT) -- (INV);
    \draw [->, ultra thick] (INV) -- (TERM);
    \draw [->, ultra thick] (TERM) -- node {\texttt{TRUE}} (ITER);
    \draw [->, ultra thick] (ITER) -| (INV);
    \draw [->] (TERM) -- node {\texttt{FALSE}} (END);

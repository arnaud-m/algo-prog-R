...
\begin{frame}[fragile]
  ...
  \tikzstyle{block} = [draw, fill=grey!5, rectangle, minimum height=3em, minimum width=6em]
...
\begin{tikzpicture}[auto, node distance=2cm,>=latex']
    ...
    \node [block, align = left] (INIT) {\lstinline[basicstyle=\normal\ttfamily]{i <- 0}\\\lstinline[basicstyle=\normal\ttfamily]{acc <- 0}};
    \node [sum, right of=INIT, node distance=2cm] (INV) {INV};
    \node [block, diamond, right of=INV, , node distance=2cm] (TERM) {\lstinline[basicstyle=\normal\ttfamily]{i < n ?}};
    ...
    \node [block, align = left, below of=TERM, node distance=2cm] (ITER) {\lstinline[basicstyle=\normal\ttfamily]{i <- i + 1}\\\lstinline[basicstyle=\normal\ttfamily]{acc <- acc + i}};% {$\mathtt{i} = \mathtt{i} + 1$\\$\mathtt{acc}=\mathtt{acc}+\mathtt{i}$};
